...
#         for i, val in enumerate(shooting_star):
#             if val < 0:
#                 patterns.append({
#                     'type': 'Shooting Star',
#                     'direction': 'bearish',
...
#         for i, val in enumerate(engulfing):
#             if val > 0:
#                 patterns.append({
...
#         exit_price = self.data['Close'].iloc[end_idx]
        
#         # Calculate returns for the lookforward period
#         returns = []
#         max_profit = 0
#         max_loss = 0
...
#                 'type': 'Strong Buy',
#                 'confidence': 0.95,
#                 'reason': 'All timeframes show bullish patterns',
...
#                 'type': 'Buy',
#                 'confidence': 0.8,
#                 'reason': '3+ timeframes show bullish patterns',
...
#                 'type': 'Strong Sell',
#                 'confidence': 0.95,
#                 'reason': 'All timeframes show bearish patterns',
...
#                 'type': 'Sell',
#                 'confidence': 0.8,
#                 'reason': '3+ timeframes show bearish patterns',
...
# # --- Stock Symbol Lists ---
# POPULAR_STOCKS = {
#     'US Stocks': ['AAPL', 'MSFT', 'GOOGL', 'AMZN', 'TSLA', 'META', 'NVDA', 'NFLX', 'CRM', 'UBER'],
...
#     sorted_patterns = sorted(pattern_ranks.items(), key=lambda x: x[1]['avg_profit'], reverse=True)
#     for i, pattern in enumerate(all_patterns):
#         pattern_type = pattern['type']
...
# symbol_from_list = st.sidebar.selectbox("Popular Stocks", POPULAR_STOCKS[category])
# custom_symbol = st.sidebar.text_input("Or Enter Custom Symbol", placeholder="e.g., AAPL, TSLA.NS")

# selected_symbol = custom_symbol.upper() if custom_symbol else symbol_from_list
...
#             '1d': analysis_period,  # Daily data
#             '1wk': 'max',   # Weekly data
#             '1mo': 'max'    # Monthly data
#         }
...
#     # Current price info
#     latest_data = list(data_by_timeframe.values())[0]
...
#     with col3:
#         st.metric("52W High", f"${latest_data['High'].max():.2f}")
#     with col4:
#         st.metric("52W Low", f"${latest_data['Low'].min():.2f}")
...
#     for i, (tf, data) in enumerate(data_by_timeframe.items()):
#         patterns, performance = analyze_patterns_for_timeframe(data, tf)
...
#                     with col2:
#                         st.metric("Stop Loss", f"${stop_loss:.2f}", f"{((stop_loss-current_price)/current_price*100):+.2f}%")
#                     with col3:
#                         st.metric("Take Profit", f"${take_profit:.2f}", f"{((take_profit-current_price)/current_price*100):+.2f}%")
...
#     volatility = latest_data['Close'].pct_change().std() * np.sqrt(252) * 100  # Annualized volatility
...
#                         live_change_pct = (live_change / current_price) * 100
                        
#                         st.metric("Live Price", f"${live_price:.2f}", 
#                                 f"{live_change:+.2f} ({live_change_pct:+.2f}%)")
                        
#                         # Check for new pattern formations
#                         if len(live_data) > 50:  # Enough data for pattern detection
...
#     # Show some popular stocks' current prices
#     sample_symbols = ['AAPL', 'MSFT', 'TSLA', 'GOOGL']
#     overview_data = []
...
# # Load Indian stocks from JSON file
# @st.cache_data
# def load_indian_stocks(json_file='indian_stocks.json'):
#     try:
#         with open(json_file, 'r') as f:
#             data = json.load(f)
#         return {stock['name']: stock['symbol'] for stock in data}
#     except FileNotFoundError:
#         st.warning("indian_stocks.json not found. Using default Indian stocks.")
#         return {
#             "Reliance Industries Ltd.": "RELIANCE.NS",
#             "Jindal Photo Ltd.": "JINDALPHOT.NS",
#             "Moser Baer India Ltd.": "MOSERBAER.NS",
#             "Mazda Ltd.": "MAZDA.NS",
#             "Medi-Caps Ltd.": "MEDICAPSBE.NS"
#         }

# indian_stocks = load_indian_stocks()
# POPULAR_STOCKS['Indian Stocks (NSE)'] = list(indian_stocks.values())
...
#     """Fetch current price fresh every time without cache"""
#     ticker = yf.Ticker(symbol)
#     try:
...
# default_category_index = categories.index('Indian Stocks (NSE)') if 'Indian Stocks (NSE)' in categories else 0
# category = st.sidebar.selectbox("Category", categories, index=default_category_index)

# if category == 'Indian Stocks (NSE)':
#     # For Indian stocks, show name (symbol) in dropdown
#     indian_options = [f"{name} ({symbol})" for name, symbol in indian_stocks.items()]
#     default_indian_index = next((i for i, opt in enumerate(indian_options) if 'Reliance Industries Ltd.' in opt), 0)
#     selected_option = st.sidebar.selectbox("Indian Stocks", indian_options, index=default_indian_index)
#     symbol_from_list = indian_stocks[selected_option.split(' (')[0]] if selected_option else 'RELIANCE.NS'
# else:
#     symbol_from_list = st.sidebar.selectbox("Popular Stocks", POPULAR_STOCKS[category])
...
#     is_indian_stock = selected_symbol.endswith('.NS')
#     currency_symbol = '₹' if is_indian_stock else '$'
    
#     # Current price info - fetch fresh every time
#     current_price, prev_close = get_current_price(selected_symbol)
#     price_change = current_price - prev_close
#     price_change_pct = (price_change / prev_close) * 100 if prev_close != 0 else 0
    
#     # Use daily data for other metrics
#     if '1d' in data_by_timeframe:
#         daily_data = data_by_timeframe['1d']
#         vol = daily_data['Volume'].iloc[-1] if not daily_data.empty else 0
...
#                 # Fetch latest price fresh
#                 live_price, _ = get_current_price(selected_symbol)
#                 live_change = live_price - current_price
...
# st.markdown("""
# <div class="main-header">
#     <h4>StockPatternAI by Aselea - Advanced Market Analysis Dashboard</h4>
#     <p>Multi-Timeframe Pattern Recognition | Real-Time Signals | Historical Backtesting</p>
# </div>
# """, unsafe_allow_html=True)
...
#         with open(json_file, 'r') as f:
#             data = json.load(f)
#         # Strip .NS from symbols to get base symbol
#         return {stock['name']: stock['symbol'].replace('.NS', '') for stock in data}
#     except FileNotFoundError:
#         st.warning("indian_stocks.json not found. Using default Indian stocks.")
#         return {
...
# def get_52w_high_low(symbol: str) -> Tuple[float, float]:
#     """Fetch 52-week high and low fresh"""
#     ticker = yf.Ticker(symbol)
#     try:
...
#     # Fetch 52w high/low fresh
#     high_52w, low_52w = get_52w_high_low(selected_symbol)
    
#     # Use daily data for volume
#     if '1d' in data_by_timeframe:
#         daily_data = data_by_timeframe['1d']
#         vol = daily_data['Volume'].iloc[-1] if not daily_data.empty else 0
#     else:
...
#     """)
    ...
    # Market overview removed since focus on Indian stocks
